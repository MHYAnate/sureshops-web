...
export function useSearch() {
  ...
  // Debounce filters too to prevent intermediate API calls
  ...
  // Stable query key from serialized filters
  const queryKey = JSON.stringify(searchFilters);

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["search", queryKey],
    queryFn: () => searchService.search(searchFilters),
    // Only enable when we have something meaningful to search
    enabled:
